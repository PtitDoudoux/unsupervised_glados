...
class Kmeans:
    ...
    def __init__(self, data: np.ndarray, nb_clusters: int, random_mus=False):
        """
        Kmeans Initializers
...
class LloydKmeans(Kmeans):
    ...
    def __init__(self, data: np.ndarray, nb_clusters: int, random_mus=False):
        ...
        :param nb_clusters: The number of cluster find
        """
        super(LloydKmeans, self).__init__(data, nb_clusters, random_mus)
    ...
    def fit(self, max_iteration=100) -> None:
        ...
        while "we haven't find each mu":
            ...
            for item in self.data:
                mu_distance = ((mu_id, np.linalg.norm(item - mu.representative)) for mu_id, mu in self.mus.items())
                min_mu_id = sorted(mu_distance, key=itemgetter(1))[0][0]
                self.mus[min_mu_id].cluster_item.append(item)
                pbar.update((1 / len(self.data)) * 100)
            pbar.close()
            new_mus = {mu_id: MU(np.mean(mu.cluster_item, axis=0), list()) for mu_id, mu in self.mus.items()}
            is_finished = all(((self.mus[mu_id].representative == new_mus[mu_id].representative).all()
                               for mu_id in self.mus.keys()))
